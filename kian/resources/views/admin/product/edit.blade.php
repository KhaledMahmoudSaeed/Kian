...
@section('content')
    <div class="container">
        <h1 class="text-center text-info my-4">@lang('messages.EDIT_PRODUCT')</h1>

        <a href="/productdashboard" class="btn btn-secondary mb-3">@lang('messages.BACK_TO_DASHBOARD')</a>

        <div id="message" class="alert d-none"></div>

        <form id="editProductForm" action="{{ route('productdashboard.update', $product->id) }}" method="POST"
            ...
            <div class="mb-3">
                ...
                <input type="text" name="name" class="form-control @error('name') is-invalid @enderror"
                    id="productName" value="{{ old('name', $product->name) }}" required>
                @error('name')
            ...
            <div class="mb-3">
                ...
                <input type="number" name="price" class="form-control @error('price') is-invalid @enderror"
                    id="productPrice" value="{{ old('price', $product->price) }}" required>
                @error('price')
                    <span class="text-danger">{{ $message }}</span>
                @enderror
            </div>
            ...
            <div class="mb-3">
                ...
                <input type="number" name="quantity" class="form-control @error('quantity') is-invalid @enderror"
                    id="productQuantity" value="{{ old('quantity', $product->quantity) }}" required>
                @error('quantity')
                    <span class="text-danger">{{ $message }}</span>
                @enderror
            </div>
            ...
            <div class="mb-3">
                ...
                <input type="number" name="sale" class="form-control @error('sale') is-invalid @enderror"
                    id="productSale" value="{{ old('sale', $product->sale) }}" required>
                @error('sale')
                    <span class="text-danger">{{ $message }}</span>
                @enderror
            </div>
            ...
            <div class="mb-3">
                ...
                @if ($product->img)
                    <div class="mt-2">
                        <img src="{{ asset('products/img/' . $product->img) }}" alt="@lang('messages.CURRENT_PRODUCT_IMAGE')"
            ...
            <div class="mb-3">
                ...
                <input type="text" name="shiper_id" class="form-control @error('shiper_id') is-invalid @enderror"
                    id="shiperId" value="{{ old('shiper_id', $product->shiper_id) }}">
                @error('shiper_id')
                    <span class="text-danger">{{ $message }}</span>
                @enderror
            </div>
